...
import { OfflineManagementService } from 'src/app/services/offline-management/offline-management.service';
...
@Component({
  ...
  styleUrls: ['./offline-table.component.css']
})
export class OfflineTableComponent implements OnInit {
   ...
   THeaders: any[] = [
   // { Key: 'Patient_Local', ShortKey: 'Patient_LocalSort', Name: 'Patient Local', If_Short: false, Condition: '' },
   ...
   constructor(public ModalService: BsModalService,
               public OfflineService: OfflineManagementService,
 ...
 ClearInput(event: KeyboardEvent): boolean {
   ...
   Service_Loader() {
      ...
      this.TableLoader();
      this.OfflineService.AllOffline_List(Data).subscribe(response => {
   ...
   ViewPatient(index: any) {
      ...
      this.modalReference = this.ModalService.show(ModalOfflineViewComponent, Object.assign({initialState}, {ignoreBackdropClick: true, class: 'modal-lg modal-dialog-centered animated bounceInRight' }));
   ...
   ExportAll() {
      const Arr = ['Patient_Name', 'Patient_Age',  'Admission_Type', 'Hospital', 'User',
      ...
      const Headers = ['Patient Name', 'Patient Age', 'Admission Type', 'Name of the hospital', 'User',
      ...
      this.OfflineService.AllOffline_List(Data).subscribe(response => {
         if (response.Status && response.Status === true ) {
            ...
            XLSX.writeFile(wb, date + '-Offline-List.xlsx');
         } else if (!response.Status && response.ErrorCode === 400 || response.ErrorCode === 401 || response.ErrorCode === 417) {
            if (response.ErrorMessage === undefined || response.ErrorMessage === '' || response.ErrorMessage === null) {
               response.ErrorMessage = 'Some Error Occoured!, But not Identified.';
            }
